...
mkdir -p "$BACKUP_DIR" && touch "$LOG_FILE" "$ROLLBACK_FILE" "$EXECUTED_STEPS_FILE"

step_mark_executed() { echo "$1" >> "$EXECUTED_STEPS_FILE"; }
step_was_executed()  { grep -qFx "$1" "$EXECUTED_STEPS_FILE" 2>/dev/null; }

require_root() { if [[ $EUID -ne 0 ]]; then error "需 root 运行。请使用 sudo。"; exit 1; fi; }
log()    { local msg="[$(date '+%Y-%m-%d %H:%M:%S')] $*"; echo "$msg" >> "$LOG_FILE"; echo "$msg"; }
safe_run() { local d="$1"; shift; log "执行：$d ($*)"; if "$@" >> "$LOG_FILE" 2>&1; then log "OK 成功：$d"; return 0; else local ec=$?; warn "FAIL 失败：$d ($ec)"; return $ec; fi; }
require_run() { local d="$1"; shift; safe_run "$d" "$@" || { error "关键步骤失败，退出。"; exit 1; }; }
...
step_sysinfo() {
    ...
    log "采集系统信息…"
    local public_ip="" ipv4="" ipv6="" isp="" country="" city=""
    ...
    if [[ -n "$json" ]]; then
        ...
        public_ip=$(echo "$json" | grep '"ip"' | awk -F': "' '{print $2}' | tr -d '",')
    fi
    if echo "$isp" | grep -Eiq 'CHINANET|mobile|unicom|telecom'; then
        ipv4=$(ip route get 8.8.8.8 2>/dev/null | grep -oP 'src \K[^ ]+' || hostname -I 2>/dev/null | awk '{print $1}')
    else
        ipv4="$public_ip"
    fi
    ipv6=$(curl -s --max-time 2 https://v6.ipinfo.io/ip 2>/dev/null) || ipv6=""
    local r=$(awk 'BEGIN{rx=0;tx=0} $1~/^(eth|ens|enp|eno)[0-9]+/{rx+=$2;tx+=$10} END{printf "%.0f %.0f", rx, tx}' /proc/net/dev 2>/dev/null)
    local rb=$(echo "$r" | awk '{print $1}') tb=$(echo "$r" | awk '{print $2}')
    ...
    if [[ -n "$s1" ]]; then sleep 1 2>/dev/null || true; local s2=$(grep 'cpu ' /proc/stat 2>/dev/null) || s2=""; [[ -n "$s2" ]] && cu=$(awk '{u=$2+$4;t=$2+$4+$5;if(NR==1){u1=u;t1=t}else printf "%.1f",(($2+$4-u1)*100/(t-t1))}' <(echo "$s1") <(echo "$s2")) || cu="0"; fi
    local cf=$(grep "MHz" /proc/cpuinfo 2>/dev/null | head -1 | awk '{printf "%.1f GHz", $4/1000}') || cf="?"; local ca=$(uname -m)
    local mi=$(free -b 2>/dev/null | awk 'NR==2{printf "%.2f/%.2fM (%.1f%%)", $3/1024/1024, $2/1024/1024, $3*100/$2}') || mi="?"
    local si=$(free -m 2>/dev/null | awk 'NR==3{used=$3;total=$2;if(total==0){p=0}else{p=used*100/total}; printf "%dM/%dM (%d%%)", used, total, p}') || si="?"
    local di=$(df -h / 2>/dev/null | awk 'NR==2{printf "%s/%s (%s)", $3, $2, $5}') || di="?"
    local hn=$(uname -n) kv=$(uname -r); local os=$(grep PRETTY_NAME /etc/os-release 2>/dev/null | cut -d '=' -f2 | tr -d '"') || os="?"
    local ld=$(uptime 2>/dev/null | awk '{print $(NF-2), $(NF-1), $NF}') || ld="?"
    local rt=$(awk '{d=int($1/86400);h=int(($1%86400)/3600);m=int(($1%3600)/60);if(d>0)printf "%d天 ",d;if(h>0)printf "%d时 ",h;printf "%d分"}' /proc/uptime 2>/dev/null) || rt="?"
    ...
    local tz=$(timedatectl 2>/dev/null | grep "Time zone" | awk '{print $3}') || tz=$(date +"%Z %z")
    local ct=$(date "+%Y-%m-%d %I:%M %p")
    local tc=$(ss -t 2>/dev/null | wc -l) || tc="?"; local uc=$(ss -u 2>/dev/null | wc -l) || uc="?"
    clear
    echo -e "${CYAN}═══════════════════════════════════════════════${NC}"
    echo -e "${CYAN}      系统信息查询${NC}"
    echo -e "${CYAN}═══════════════════════════════════════════════${NC}"
    echo -e "${CYAN}主机名:         ${NC}$hn"; echo -e "${CYAN}系统版本:       ${NC}$os"
    echo -e "${CYAN}Linux版本:      ${NC}$kv"; echo -e "${CYAN}CPU架构:        ${NC}$ca"
    echo -e "${CYAN}CPU型号:        ${NC}$ci"; echo -e "${CYAN}CPU核心数:      ${NC}$cc"
    echo -e "${CYAN}CPU频率:        ${NC}$cf"; echo -e "${CYAN}CPU占用:        ${NC}${cu}%"
    echo -e "${CYAN}系统负载:       ${NC}$ld"; echo -e "${CYAN}TCP/UDP连接:    ${NC}${tc}/${uc}"
    echo -e "${CYAN}物理内存:       ${NC}$mi"; echo -e "${CYAN}虚拟内存:       ${NC}$si"
    echo -e "${CYAN}硬盘占用:       ${NC}$di"; echo -e "${CYAN}总接收/总发送:  ${NC}$rx / $tx"
    echo -e "${CYAN}网络算法:       ${NC}$cg $qa"; echo -e "${CYAN}运营商:         ${NC}${isp:-N/A}"
    echo -e "${CYAN}IPv4地址:       ${NC}${ipv4:-N/A}"; echo -e "${CYAN}IPv6地址:       ${NC}${ipv6:-N/A}"
    echo -e "${CYAN}DNS地址:        ${NC}$dns"; echo -e "${CYAN}地理位置:       ${NC}${country:-?} ${city:-?}"
    echo -e "${CYAN}时区/时间:      ${NC}$tz $ct"; echo -e "${CYAN}运行时长:       ${NC}$rt"
    echo -e "${CYAN}═══════════════════════════════════════════════${NC}"
...
step_system_update() {
    ...
    require_run "apt update" apt-get update; require_run "apt upgrade -y" apt-get upgrade -y; require_run "apt dist-upgrade -y" apt-get dist-upgrade -y
    safe_run "孤立依赖" apt-get autoremove -y; step_mark_executed "system_update"; info "系统更新完成。"
}
...
step_install_packages() {
    ...
    local failed=()
    for pkg in "${pkgs[@]}"; do dpkg -s "$pkg" &>/dev/null && log "  已安装：$pkg" || { safe_run "安装 $pkg" apt-get install -y "$pkg" || failed+=("$pkg"); }; done
    [[ ${#failed[@]} -gt 0 ]] && safe_run "重试" apt-get --fix-missing install -y "${failed[@]}" || warn "仍有失败：${failed[*]}"
...
step_install_bbr() {
    ...
    require_run "下载 BBR 脚本" wget -q -O "$bs" "https://raw.githubusercontent.com/Snorbo/public/refs/heads/main/2026newconfig/bbr.sh"
    ...
    safe_run "执行 BBR 脚本" bash "$bs" 1 || { warn "脚本异常，手动开启…"; echo "net.core.default_qdisc=fq" >> /etc/sysctl.conf 2>/dev/null || true; echo "net.ipv4.tcp_congestion_control=bbr" >> /etc/sysctl.conf 2>/dev/null || true; sysctl -p 2>/dev/null || true; }
    local ba=$(sysctl -n net.ipv4.tcp_congestion_control 2>/dev/null) || ba="?"
    lsmod 2>/dev/null | grep -q tcp_bbr && info "OK BBR 已加载（$ba）" || warn "BBR 未加载，需重启。当前算法：$ba"
...
step_change_ssh_port() {
    ...
    systemctl is-active ssh.socket 2>/dev/null | grep -q active && socket_active=true
    systemctl is-active sshd.socket 2>/dev/null | grep -q active && socket_active=true
    if $socket_active; then
        info "检测到 systemd socket activation（ssh.socket/sshd.socket），禁用中…"
        log "禁用 socket activation 以确保 Port 指令生效"
    ...
    # ── 防火墙放行 ──
    command -v ufw &>/dev/null && ufw status 2>/dev/null | grep -q active && { ufw allow "$new_port/tcp" 2>/dev/null || true; ufw status 2>/dev/null | grep -q "$new_port" && info "UFW 已放行 $new_port/tcp" || warn "UFW 规则可能未生效。"; }
    command -v firewall-cmd &>/dev/null && firewall-cmd --state 2>/dev/null | grep -q running && { firewall-cmd --permanent --add-port="${new_port}/tcp" 2>/dev/null || true; firewall-cmd --reload 2>/dev/null || true; firewall-cmd --list-ports 2>/dev/null | grep -q "$new_port" && info "firewalld 已放行 $new_port/tcp" || warn "firewalld 规则可能未生效。"; }
    command -v iptables &>/dev/null && { local dp=$(iptables -L INPUT -n 2>/dev/null | head -1 | awk '{print $4}' | tr -d '()'); [[ "$dp" == "DROP" || "$dp" == "REJECT" ]] && { iptables -I INPUT -p tcp --dport "$new_port" -j ACCEPT 2>/dev/null || true; info "已添加 iptables 放行规则。"; }; }

    # ── 重启 SSH ──
    if command -v sshd &>/dev/null; then
        local syntax_ok
        syntax_ok=$(sshd -t 2>&1) || {
            error "sshd_config 语法错误："; echo "$syntax_ok" | while IFS= read -r err; do echo "  $err"; done
            warn "回滚…"; cp "${BACKUP_DIR}/sshd_config.bak" /etc/ssh/sshd_config; return 1
        }
        log "sshd_config 语法检查通过。"
    fi
    local ssh_ok=true
    systemctl restart sshd 2>/dev/null || service ssh restart 2>/dev/null || ssh_ok=false
    if ! $ssh_ok; then
        error "SSH 重启失败："; systemctl status sshd --no-pager 2>&1 | head -10 | while IFS= read -r l; do log "  $l"; done
        warn "回滚…"; cp "${BACKUP_DIR}/sshd_config.bak" /etc/ssh/sshd_config
        systemctl restart sshd 2>/dev/null || service ssh restart 2>/dev/null || true; return 1
    fi
    sleep 2
    local listening=$(ss -tlnp 2>/dev/null | grep sshd) || listening=""
    if echo "$listening" | grep -q ":${new_port} "; then
        info "OK 确认 SSH 正在监听端口 $new_port"
    else
        warn "SSH 未在 $new_port 上监听！当前监听："
        [[ -n "$listening" ]] && echo "$listening" | while IFS= read -r l; do log "  $l"; done || log "  sshd 未监听任何端口"
        pgrep -x sshd &>/dev/null && log "  sshd 进程存在" || { log "  sshd 进程不存在"; systemctl status sshd --no-pager 2>&1 | tail -5 | while IFS= read -r l; do log "  $l"; done; }
    fi
...
# ══════════ 步骤 7：SSH 密钥配置 ══════════════════════════════════════════
step_ssh_key_config() {
    step_header "步骤 7/10：禁用密码登录，配置 SSH 密钥"
    require_root; local sc="/etc/ssh/sshd_config"
    [[ ! -f "$sc" ]] && { error "未找到 $sc"; return 1; }
    [[ ! -f "${BACKUP_DIR}/sshd_config.bak" ]] && { cp "$sc" "${BACKUP_DIR}/sshd_config.bak"; register_rollback "cp '${BACKUP_DIR}/sshd_config.bak' '$sc'"; register_rollback "systemctl restart sshd || service ssh restart || true"; }
    local key="${SSH_PUBKEY:-}"
    if [[ -z "$key" ]]; then
        echo -e "${YELLOW}粘贴公钥，新行 EOF 结束（回车跳过）：${NC}"; key=""
        while IFS= read -r line; do [[ "$line" == "EOF" ]] && break; [[ -z "$line" && -z "$key" ]] && break; key+="$line"$'\n'; done
        key="$(echo -e "$key" | sed '/^$/d')"
    else info "使用 SSH_PUBKEY 环境变量。"; fi
    if [[ -n "$key" ]]; then
        mkdir -p /root/.ssh; chmod 700 /root/.ssh; echo "$key" >> /root/.ssh/authorized_keys; sort -u /root/.ssh/authorized_keys -o /root/.ssh/authorized_keys; chmod 600 /root/.ssh/authorized_keys
        [[ -n "${SUDO_USER:-}" && "$SUDO_USER" != "root" ]] && { local uh=$(getent passwd "$SUDO_USER" | cut -d: -f6) || uh="/home/$SUDO_USER"; [[ -n "$uh" ]] && { mkdir -p "$uh/.ssh"; chmod 700 "$uh/.ssh"; echo "$key" >> "$uh/.ssh/authorized_keys"; sort -u "$uh/.ssh/authorized_keys" -o "$uh/.ssh/authorized_keys"; chmod 600 "$uh/.ssh/authorized_keys"; chown -R "$SUDO_USER:$(id -gn "$SUDO_USER")" "$uh/.ssh" 2>/dev/null || true; }; }
        info "公钥已添加。"
    else info "跳过。"; fi
    ...
    if [[ "${dp,,}" == "yes" || "${dp,,}" == "y" ]]; then
        ...
        [[ "$kc" -eq 0 ]] && { warn "无公钥！"; read -r -p "确认继续？(yes/NO): " fd; [[ "${fd,,}" != "yes" ]] && { info "已取消。"; return 0; }; }
        ...
        log "已配置：密码禁用，密钥启用。"; systemctl restart sshd 2>/dev/null || service ssh restart 2>/dev/null || true; info "SSH 已重启。"
        echo -e "${YELLOW}保持当前会话，另开终端测试密钥登录。${NC}"
        read -r -p "确认正常？(yes/NO): " ck; [[ "$ck" != "yes" ]] && { warn "回滚…"; cp "${BACKUP_DIR}/sshd_config.bak" "$sc"; systemctl restart sshd 2>/dev/null || service ssh restart 2>/dev/null || true; error "已回滚。"; return 1; }
    else info "跳过。"; fi
...
step_block_ipqs() {
    ...
    local a=0; for e in "${es[@]}"; do grep -qF "$e" "$hf" 2>/dev/null && log "  已存在：$e" || { echo "$e" >> "$hf"; log "  已添加：$e"; ((a++)); }; done
    info "IPQS 屏蔽完成（新增 $a 条）。"; step_mark_executed "block_ipqs"
}

# ══════════ 步骤 9：解除 53 端口占用 ══════════════════════════════════════
step_release_port53() {
    step_header "步骤 9/10：解除 53 端口占用"
    require_root; local rc="/etc/systemd/resolved.conf"
    [[ -f "$rc" ]] || touch "$rc"
...
EOL
    ...
    echo ""; info "53 端口："; command -v lsof &>/dev/null && lsof -i :53 2>/dev/null || ss -tlnp 2>/dev/null | grep ':53 ' || echo "  (无进程监听 53)"
    echo ""; info "DNS 测试："; safe_run "DNS" nslookup google.com 8.8.8.8 2>/dev/null || safe_run "DNS" dig +short google.com @8.8.8.8 2>/dev/null || warn "DNS 测试失败。"
    step_mark_executed "release_port53"; info "53 端口配置完成。"
}

# ══════════ 步骤 10：系统清理 ═════════════════════════════════════════════
step_system_cleanup() {
    step_header "步骤 10/10：系统清理"
    require_root
    warn "深度清理：旧内核、孤立包、apt 缓存、残留配置、日志、tmp、snap、pip/npm。"
    local c; read -r -p "确认？(yes/NO): " c; [[ "${c,,}" != "yes" ]] && { info "跳过。"; return 0; }; echo ""
    info "--- 1/8 旧内核 ---"
    command -v dpkg &>/dev/null && {
        ...
        for pkg in "${imgs[@]}"; do local ver=$(echo "$pkg" | sed 's/^linux-image-//; s/^linux-image-unsigned-//'); [[ "$ver" == "$rk" ]] && ((keep++)); ((i < cnt - keep)) && purge+=("$pkg"); ((i++)); done
        if [[ ${#purge[@]} -gt 0 ]]; then log "可清理 ${#purge[@]} 个"; local ck; read -r -p "移除？(yes/NO): " ck; [[ "${ck,,}" == "yes" ]] && safe_run "移除旧内核" apt-get purge -y "${purge[@]}" || info "跳过。"; fi
        local hdrs=(); while IFS= read -r p; do hdrs+=("$p"); done < <(dpkg -l 'linux-headers-*' 2>/dev/null | awk '/^ii/ {print $2}' | sort -V | head -n -1)
        local hp=(); for pkg in "${hdrs[@]}"; do local ver=$(echo "$pkg" | sed 's/^linux-headers-//'); [[ "$ver" != "$rk" && "$ver" != "generic" ]] && hp+=("$pkg"); done
        [[ ${#hp[@]} -gt 0 ]] && safe_run "移除旧 headers" apt-get purge -y "${hp[@]}" 2>/dev/null || true
    }
    ...
    [[ "$rc" -gt 0 ]] && { dpkg -l | awk '/^rc/ {print $2}' | xargs -r dpkg --purge 2>/dev/null || true; info "已清理 $rc 个。"; } || info "无残留。"
    echo ""; info "--- 5/8 Snap ---"
    command -v snap &>/dev/null && { local sn=$(snap list --all 2>/dev/null | awk '/disabled/ {print $1, $3}') || sn=""; [[ -n "$sn" ]] && { echo "$sn"; local cs; read -r -p "移除？(yes/NO): " cs; [[ "${cs,,}" == "yes" ]] && echo "$sn" | while IFS=' ' read -r n r; do safe_run "移除 snap $n ($r)" snap remove "$n" --revision="$r" 2>/dev/null || true; done; }; } || info "Snap 未安装。"
    echo ""; info "--- 6/8 日志 ---"
...
# ─── 流程内清理 ─────────────────────────────────────────────────────────────
step_cleanup() { step_header "清理临时文件"; rm -f "${BACKUP_DIR}/bbr.sh" 2>/dev/null || true; log "备份：$BACKUP_DIR"; local sz=$(du -sh "$BACKUP_DIR" 2>/dev/null | awk '{print $1}') || sz="N/A"; info "备份大小：$sz"; }

# ─── 全局回滚 ────────────────────────────────────────────────────────────────
step_global_rollback() {
    step_header "全局回滚"
    require_root; [[ ! -f "$ROLLBACK_FILE" ]] && { info "无回滚记录。"; return 0; }
    local cnt=$(wc -l < "$ROLLBACK_FILE"); [[ "$cnt" -eq 0 ]] && { info "无回滚操作。"; return 0; }
    warn "执行 $cnt 条回滚…"; tac "$ROLLBACK_FILE" | while IFS= read -r cmd; do [[ -n "$cmd" ]] && bash -c "$cmd" >> "$LOG_FILE" 2>&1 || true; done
...
show_menu() {
    ...
    echo "  3) 安装常用包      4) 安装 BBR"
    echo "  5) 安装 NextTrace  6) 修改 SSH 端口"
    echo "  7) SSH 密钥配置    8) 禁止 IPQS"
    echo "  9) 解除 53 端口    10) 系统深度清理"
...
# ─── 主流程 ──────────────────────────────────────────────────────────────────
main() {
    require_root; local run_all=false sel=""
    while [[ $# -gt 0 ]]; do case "$1" in --all) run_all=true; shift;; --step) sel="${2:-}"; shift 2;; --rollback) step_global_rollback; exit 0;; -h|--help) show_menu; exit 0;; *) error "未知：$1"; exit 1;; esac; done
    ...
    if [[ -n "$sel" ]]; then
        log "步骤 $sel"; IFS=',' read -ra steps <<< "$sel"
        for s in "${steps[@]}"; do s="$(echo "$s" | xargs)"; case "$s" in 1) step_sysinfo;; 2) step_system_update;; 3) step_install_packages;; 4) step_install_bbr;; 5) step_install_nexttrace;; 6) step_change_ssh_port;; 7) step_ssh_key_config;; 8) step_block_ipqs;; 9) step_release_port53;; 10) step_system_cleanup;; *) warn "跳过：$s";; esac; done
        step_cleanup; info "完成！备份：$BACKUP_DIR"; return 0
    fi
    show_menu; read -r -p "请选择 [1-10,a,r,q]: " choice
    case "$choice" in a|A) main --all;; r|R) step_global_rollback;; q|Q) info "退出。"; exit 0;;
        *) IFS=',; ' read -ra selections <<< "$choice"
           for sel in "${selections[@]}"; do sel="$(echo "$sel" | xargs)"; case "$sel" in 1) step_sysinfo;; 2) step_system_update;; 3) step_install_packages;; 4) step_install_bbr;; 5) step_install_nexttrace;; 6) step_change_ssh_port;; 7) step_ssh_key_config;; 8) step_block_ipqs;; 9) step_release_port53;; 10) step_system_cleanup;; *) warn "跳过：$sel";; esac; done
